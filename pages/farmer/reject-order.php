...
if (!is_logged_in() || $_SESSION['role'] !== 'farmer') {
    set_flash_message('error', 'Please login as a farmer to reject orders');
...
try {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $message = "Your order #" . $order_id . " for " . $order['product_name'] . " has been rejected by the farmer. Please try ordering from another farmer.";
        
        // Log the SMS (in real implementation, you would send actual SMS)
        error_log("SMS to " . $order['buyer_phone'] . ": " . $message);

        set_flash_message('success', 'Order rejected successfully. SMS notification sent to buyer.');
    } else {
        set_flash_message('error', 'Order could not be rejected. It may have already been processed.');
    }
